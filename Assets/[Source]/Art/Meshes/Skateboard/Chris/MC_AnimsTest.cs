...
public class MC_AnimsTest : MonoBehaviour
{
    #region Variables
    ...
    [SerializeField] private AnimationCurve 
        ...
        jumpCurve = new AnimationCurve(new Keyframe[]{new Keyframe(time: 0f, value: 0f), new Keyframe(time: .5f, value: 1f), new Keyframe(time: 1, value: 0)});
    ...
    private void DoJumping()
    {
        ...
        if(Input.GetKeyUp(KeyCode.Space) && _chargingJump)
        {
            _idle = false;
            _accelerate = false;
            _cruise = false;
        ...
        float __CalcCurrentJump()
        {
            _timeJumping += Time.deltaTime;
            
            _jumpingEvaluation = (_timeJumping / timeTillJumpApex).Clamp01();
            
            float __lerpValue = jumpCurve.Evaluate(_jumpingEvaluation);
        ...
        bool __Landing()
        {
            if(_jumpDirection != JumpDirection.Down || !currentJump.Approximately(0)) return false;
