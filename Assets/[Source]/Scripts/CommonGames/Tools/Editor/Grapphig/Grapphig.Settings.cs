using System;
using System.Collections;
using System.Collections.Generic;
using UnityEditor;
using UnityEngine;

namespace CommonGames.Tools
{
    public partial class Grapphig
    {
        [Serializable]
        public static class GraphSettings
        {
            // Positions
            public static Vector4 graphMargins = new Vector4(0f, 1f, 305f, 40f);

            // UI
            public const float SLIDER_SENSITIVITY = 0.5f;
            public const float BUTTON_SIZE = 18f;

            // Graph 
            public const bool
                SHOW_HORIZONTAL_RULER = false,
                SHOW_VERTICAL_RULER = false,
                SHOW_GRID_BACKGROUND = true;

            // Colors
            public static Color32 windowBackgroundColor = new Color32(162, 162, 162, 255);
            public static Color32 borderBackgroundColor = new Color32(194, 194, 194, 255);
            public static Color32 graphBackgroundColor = Color.red; //new Color32(80, 100, 100, 255);
            public static Color32 checkerDarkColor = new Color32(30, 30, 30, 255);
            public static Color32 checkerLightColor = new Color32(41, 41, 41, 255);
            public static Color32 graphStatColor = new Color(0, 0, 0, 0.4f);
            public static Color32 ruleLineColor = new Color32(255, 255, 255, 100);
            public static Color32 gridLineColor = new Color32(255, 255, 255, 80);

            public static Color32 panelBackgroundColor = new Color32(240, 240, 240, 255);
            public static Color32 panelHeaderColor = new Color32(220, 220, 220, 255);
            public static Color32 buttonBackgroundColor = new Color32(220, 220, 220, 255);
            public static Color32 buttonHoverColor = new Color32(180, 180, 180, 255);
            public static Color32 buttonActiveColor = new Color32(130, 255, 150, 255);
    
            //public static float autoScalePercent = 0.9f;

            public static float HorizontalResolution
            {
                get => EditorPrefs.GetFloat("GrapphigHorizontalResolution", 8);
                set => EditorPrefs.SetFloat("GrapphigHorizontalResolution", value);
            }

            public static int SharedVerticalResolution
            {
                get => EditorPrefs.GetInt("GrapphigSharedVerticalResolution", 0);
                set => EditorPrefs.SetInt("GrapphigSharedVerticalResolution", value);
            }

            public static int OverwriteFiles
            {
                get => EditorPrefs.GetInt("GrapphigOverwriteFiles", 0);
                set => EditorPrefs.SetInt("GrapphigOverwriteFiles", value);
            }

            public static int GraphLineStyle
            {
                get => EditorPrefs.GetInt("GrapphigLineStyle", 0);
                set => EditorPrefs.SetInt("GrapphigLineStyle", value);
            }

            public static float DefaultVerticalResolution
            {
                get => EditorPrefs.GetFloat("GrapphigDefaultVerticalResolution", 10);
                set => EditorPrefs.SetFloat("GrapphigDefaultVerticalResolution", value);
            }

            public static int DefaultLogToFile
            {
                get => EditorPrefs.GetInt("GrapphigLogToFileDefault", 0);
                set => EditorPrefs.SetInt("GrapphigLogToFileDefault", value);
            }

            public static int DefaultLogToConsole
            {
                get => EditorPrefs.GetInt("GrapphigLogToConsoleDefault", 0);
                set => EditorPrefs.SetInt("GrapphigLogToConsoleDefault", value);
            }

            public static Texture2D GetTextureFrom64(string b64)
            {
                Texture2D __texture = new Texture2D(1, 1);
                byte[] __b64Bytes = System.Convert.FromBase64String(b64);
                __texture.LoadImage(__b64Bytes);
                __texture.Apply();
                return __texture;
            }
            
            public static string scaleIcon64 = @"";

            public static string logIcon64 =
                @"iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8 / 9hAAAABHNCSVQICAgIfAhkiAAAAAlwSFlzAAAEhQ
                ";

            public static string consoleIcon64 = @"";

            public static string showIcon64 =
                @"";
        }

    }
}
