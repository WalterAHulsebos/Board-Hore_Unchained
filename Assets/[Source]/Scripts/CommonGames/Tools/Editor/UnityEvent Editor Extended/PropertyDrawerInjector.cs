...
namespace Coffee.EditorExtensions
{
	public static class PropertyDrawerInjector
	{
		...
		{
			// Find all drawers.
			foreach (Type drawerType in loadedTypes.Where(x => x.IsSubclassOf(typeof(GUIDrawer))))
			{
		...
		public static void InjectPropertyDrawer(Type drawerType, InjectablePropertyDrawer attr)
		{
			...
			{
				foreach (Type type in loadedTypes.Where(x => x.IsSubclassOf(attr.type)))
				{
		...
		// Private Members.
		//################################
		private static IDictionary _s_DicDrawerTypeForType;
		private static Type[] _s_LoadedTypes;
	}
}