			...
			try
			{
				...
				while (0 < path.Length)
				{
					...
					if ((match = s_RegexArray.Match(path)).Success)
					{
						...
						MethodInfo mi = instance.GetMemberInfo(instance is Array ? "Get" : "get_Item", MemberTypes.Method) as MethodInfo;
					...
					{
						FieldInfo fi = instance.GetMemberInfo(match.Groups[1].Value, MemberTypes.Field) as FieldInfo;
					...
					{
						FieldInfo fi = instance.GetMemberInfo(path, MemberTypes.Field) as FieldInfo;
						getter = x => fi.GetValue(preGetter(x));
						break;
					}
				}
