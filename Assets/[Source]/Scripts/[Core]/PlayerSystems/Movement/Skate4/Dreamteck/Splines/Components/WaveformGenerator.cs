...
namespace Dreamteck.Splines
{
    ...
    public class WaveformGenerator : MeshGenerator
    {
        ...
        {
            AllocateMesh(clippedSamples.Length * (_slices + 1) * 2, _slices * (clippedSamples.Length - 1) * 6);
            int vertIndex = 0;
            float avgTop = 0f;
            float avgBottom = 0f;
            for (int i = 0; i < clippedSamples.Length; i++)
            ...
            if (clippedSamples.Length > 0) avgTop /= clippedSamples.Length;
            MeshUtility.GeneratePlaneTriangles(ref tsMesh.triangles, _slices * 2 - 1, clippedSamples.Length, avgTop * 2f < avgBottom);
        }
