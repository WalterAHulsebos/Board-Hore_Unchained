...
namespace RootMotion.FinalIK {
	...
	public class BodyTilt: OffsetModifier {
		...
		protected override void OnModifyOffset()
		{
			...
			Quaternion _change = Quaternion.FromToRotation(lastForward, transform.forward);

			// ReSharper disable once InconsistentNaming
			_change.ToAngleAxis(out float deltaAngle, out Vector3 _axis);
			if (_axis.y > 0)
			{
			...
			float _tiltWeight = Mathf.Abs(tiltAngle); //Mathf.Abs(tiltAngle) / 1f;
			if (tiltAngle < 0)
			{
				poseRight.Apply(ik.solver, _tiltWeight);
			}
			else
			{
				poseLeft.Apply(ik.solver, _tiltWeight);
			}
